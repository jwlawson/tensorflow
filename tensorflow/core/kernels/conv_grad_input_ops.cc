...
class Conv2DFastBackpropInputOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    size_t out_bp_other = dims.spatial_dims[0].output_size * dims.spatial_dims[1].output_size * dims.out_depth;
    ...
    for (size_t i = 0; i < num_launches; i++) {
      ...
      Tensor in_split = in_backprop->Slice(start, end);
      const Tensor out_split = out_backprop.Slice(start, end);

    LaunchBackwardInputConvolution<Device, T>()(
        context, context->eigen_device<Device>(), in_split.tensor<T, 4>(),
        filter.tensor<T, 4>(), out_split.tensor<T, 4>(),
